...
`pnpm dev` 会把扩展注册进 Raycast 并启动监听。等命令在 Raycast 里出现后按 <kbd>Ctrl</kbd>+<kbd>C</kbd> 退出——**扩展会保留**，不需要一直开着 dev server。之后搜 `unquarantine` 就能用。

需要 macOS、[Raycast](https://raycast.com)、Node.js 22+ 和 [pnpm](https://pnpm.io) 11+。想卸载就用 Raycast 的 *Manage Extensions*。
...
它只看**最近 24 小时内下载**、且真正会被拦的 `.app`，没有下载时间戳的一律排除。什么都没匹配上就提示「没有可解锁的 App」——更早的东西交给 `Unquarantine Apps` 手动挑。
...
系统设置里的 Open Anyway 是个没有 accessibility title 的 SwiftUI 按钮，没法按名字找到。只能靠它旁边那句提示——*"X" was blocked to protect your Mac.*——来定位，App 名字也是从这句话里解析出来的。
