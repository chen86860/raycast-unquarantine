...
| 设置               | 默认     | 作用                                                                    |
| ------------------ | -------- | ----------------------------------------------------------------------- |
| 界面语言           | 跟随系统 | 提示与对话框用 English 或简体中文                                       |
...
提示、对话框、列表标签都是双语的。「跟随系统」会读 macOS 的偏好语言列表，首选是中文就用中文；下拉可以强制指定。

命令名、命令描述和设置项标题本身固定为英文，这不是偷懒：Raycast 没有 i18n 能力。`environment` 只暴露 `appearance` 和 `textSize`，没有 locale；[manifest schema](https://www.raycast.com/schemas/extension.json) 里每个 `title` / `description` 都只接受单个字符串。做了本地化的扩展全是这个路子——自带语言包，外加一个选语言的设置项。
